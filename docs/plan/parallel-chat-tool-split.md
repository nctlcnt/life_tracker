...
- 状态：已定案（2026-08-26）；LT-169 实现规格已合并，LT-170 的共享 `GenerationGate` 与持久 outbound queue 已在 `feat/LT-170-unified-outbound-queue` 完成实现和自动验证，开关默认关闭，尚未合并或部署。第 9 节原来的四个未决问题已于 2026-08-24 全部定案，第 9.5 节异步带来的六个问题已于 2026-08-26 定案。第 5.3 节场景状态已经写成代码（见 `bot/memory/scene_state.py` 与 `track_scene` 工具）
...
> **LT-170 实现状态（2026-08-26）**：本节的 SQLite outbox、单 consumer、同频道队首阻塞、lease fencing、失败重试、共享生成 gate、关闭前排空校验和默认关闭的分阶段开关均已在功能分支实现；自动测试覆盖重启恢复、终态失败放行、跨频道等待、过期 lease、真实 Bot/Scheduler gate 接线与 2000 字分片不交错。尚未合并、启用或部署。
...
0. **异步基础设施**（第 0、2.1、2.2、4.3～4.5 节）：待处理队列与游标、统一发送队列、批次状态与幂等键、两分钟心跳。内部顺序是：**LT-169 实现规格已完成并合并 → LT-170 已在功能分支实现统一发送队列与共享生成 gate，待 PR/合并 → 再为 batch/cursor/幂等 executor/heartbeat 建实现 issue**。统一发送队列排在代码改动最前面，因为它单独就能修掉第 13 节第二条那个乱序缺陷，而且完全不依赖双层结构——现在的单模型路径也可以先用上。
