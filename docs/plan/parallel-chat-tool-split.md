...
gate 可以先用一把共享 `asyncio.Lock` 实现。持锁期间允许等待自己的 delivery 发完，因为发送 consumer **永远不获取 gate**；否则会形成 `生成 → 等发送 → 等生成锁` 的死锁。工具 worker 的执行、批次 lease 和心跳不经过这把锁，只有它产出内部结构化结果后、需要调用聊天模型表达结果时才进入 gate。
...
执行轨只返回内部结构化报告：`execution_results` 描述做了什么和结果状态，`important_information` 列出聊天轨必须准确传达的原子值。后者只放真正需要说给用户的时间、金额、名称等，不放完整句子、代词、工具名或数据库 ID。聊天轨可以调整语气，但必须逐字保留其中的 `value`。
...
| 结果   | 含义             | 聊天模型收到什么         |
| ---- | -------------- | ---------------- |
| `empty` | 当前批次不需要操作 | 不创建工具结果回复 |
| `completed` | 做了这些事 / 查到这些 | `execution_results` 和 `important_information` 进入结果队列 |
| `unable` | 没有适用的工具、缺少关键信息或执行失败 | 结构化失败结果进入结果队列，由聊天模型如实说明或追问 |

执行轨的报告是 bot 自己的脑内执行结果，**不是另一个 speaker 的话，也不是可以直接发送的文案**。结果表达仍由聊天轨完成；`important_information[].value` 必须逐字保留。若表达遗漏重要值、输出工具协议或泄露内部 ID，该结果不得发送，而是进入安全重试。
...
| `bot/async_pipeline/tool_worker.py` | 装配区分 `new_messages/context` 的输入、调用工具模型、经幂等 executor 执行、产出内部结构化结果 |
...
### 省 token 的关键：内部结构化结果是一条天然的懒加载通道
...
例如 reminder 和 deadline 都不需要常驻聊天模型的上下文。用户明确问起时，由工具模型查询并通过内部结构化结果返回；工具模型刚完成写入时，也由同一结构提供结果。原来要求聊天模型在用户说"不想做"时主动读取 deadline 的规则不再保留。
...
| 重要信息保真 | `important_information[].value` 漂移 = 0 | 离线 replay | 聊天模型的润色冲动更强，标准应比老设计的 5% 更严 |
...
离线 replay 只验证模型输入输出，包括漏判、误判、重要信息保真、来源约束和指令遵守，并且要能对比不同人设版本。发送顺序、不阻塞、心跳恢复、占用过期和幂等写入必须通过单元测试、集成测试或运行时测量验证，不能用离线 replay 代替。
...
**三、表达调用看到什么上下文。** 工具结果入队时重新读取最近 20 条对话消息，与现有聊天路径一致，并额外接收执行轨的内部结构化报告（`outcome`、`execution_results`、`important_information`）。这份报告是同一个 bot 的脑内活动，不是给用户看的原文；表达调用只说结果带来的新信息，不复述用户已经说过的内容，也不暴露 JSON、工具名或数据库 ID。
...
1. **人设写入数据库**（[LT-168](https://linear.app/chachas/issue/LT-168)）。第 11 节的初稿按 11.3 落位，处理那里的三个问题。这一步同时是第 5.3 节场景状态的前置——场景描述里刻意不带语气，人设必须先接住。
   ...
   **2026-08-28 更新：这一步做过一次又回滚了，issue 已退回 Backlog。** 08-27 曾把生产的
   `main_template` 从 1787 字改到 2496 字，八条禁令改为正向表述；08-28 恢复原状。回滚的原因是
   顺序：LT-168 自己写明人设一直写得短是因为单模型下人设越厚越容易干扰工具调用，而解除这个
   约束靠的正是本文件第 5 步的分层，但它同时又写着「建议先于架构做」，实际执行的是后一句。
   现在改为先做架构，等聊天一侧确实拿不到工具之后再重写人设。

   **另外 11.3 那段落位说明是错的，将来做这一步时要按实际入口改。** 它说第一部分替换
   `identity` + `communication` 两段，但自 LT-129 起真正参与渲染的只有 `main_template`
   （`bot/prompts.py:398`：`prompt_sections["main_template"] or synthesize_main_template(...)`）。
   那五段旧散文 section 只在 `main_template` 为空时作为回滚 fallback 被合成，平时是死数据。
