...
执行轨只返回内部结构化报告：`execution_results` 描述做了什么和结果状态，`important_information` 列出聊天轨可能需要的原子值。后者只放真正有用的时间、金额、名称等，不放完整句子、代词、工具名或数据库 ID。聊天轨把它当作结构化源数据，可以自然格式化，但不得改变含义。
...
执行轨的报告是 bot 自己的脑内执行结果，**不是另一个 speaker 的话，也不是可以直接发送的文案**。结果表达仍由聊天轨完成；`important_information` 是语义事实而不是逐字台词。若表达输出工具协议、原始 JSON 或泄露内部 ID，该结果不得发送，而是进入安全重试。
...
| 重要信息保真 | `important_information` 语义漂移 = 0 | 离线 replay | 允许自然格式化，但不能改时间、金额、对象或执行状态 |
