...
- [聊天与工具并行分层](parallel-chat-tool-split.md) — 设计已定案；LT-169 已合并，LT-170（统一发送队列）已在功能分支完成实现和自动验证，待 PR/合并，尚未部署。LT-174（无人回应门禁与话题消费）作为后续独立质量任务；LT-171（场景边界）、LT-172（天气快照）、LT-173（清理 Dispatch 死代码）互不依赖。
