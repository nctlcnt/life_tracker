...
TOOL_ROUND_REMINDER = (
    ...
    "继续回复时：\n"
    "1. 本轮如果还要调工具，可以继续写独白做思考，用户看不到，标签可有可无\n"
    "2. 如果本轮是最后一轮（不再调工具），文字**分两部分**：\n"
    "   - 任何自我思考 / 自检 / 对工具结果的反应（「好的，事件已记录」「她说 X，自然回应就好」"
    "这类话）→ 必须放进 `<think>...</think>` 标签里，系统会剥掉不让用户看到\n"
    "   - 标签**外**的文字 = 直接发给用户看的自然对话，像朋友发微信，不要有独白痕迹\n"
    "3. 最后一轮**务必**在标签外留一段给用户看的话！用户发了消息得不到回应会很难受。"
    "至少一句自然的回应，不要只输出思考"
)
...
- ❌ `好的，事件已经记录完整了。她说困了，自然回应就好。\n\n去睡吧，早点休息～`
  （"好的，事件..."和"她说困了，自然回应就好"是独白，必须进 `<think>` 标签，否则会被用户看到）
- ❌ `<think>她说困了</think>好的，记好了。去睡吧`（"好的，记好了"也是冗余，不要说）

**实践建议**：
- 中间轮尽量简短，只写决策和推理，能省就省
- 最后一轮想说什么就说什么，用户一定要收到回复！标签外至少留一句自然的话
