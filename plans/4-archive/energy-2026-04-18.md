...
本计划中 **chill / drain（蓄水 / 漏水）子标签**这一子目标已于 `refactor/drop-energy-type` 分支整体撤销：
- `energy_type` 字段从 DB / tools / prompts / 前端全部移除
- `ChillDrainChart` 组件删除
- 动机：用户自述无法精准自评状态，前端也一直不看这张图，保留只会增加 AI 分类负担

**保留**：Focus / Routine / Chill 三分法仍然有效（`category` 字段 + `MultiLaneTimeline` 三泳道）。

**后续方向**：精力识别相关的功能以 `plan-Merlin.md` 的离线抽取管道为主，不再在实时 prompt 里让 AI 做自评分类。
